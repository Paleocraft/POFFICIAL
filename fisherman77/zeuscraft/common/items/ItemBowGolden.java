...
public class ItemBowGolden extends Item
{
    public ItemBowGolden(int par1)
    {
        super(par1);
        this.maxStackSize = 1;
    ...
    public int getIconIndex(ItemStack stack, int renderPass, EntityPlayer player, ItemStack usingItem, int useRemaining)
    {
        if (usingItem != null && usingItem.getItem().itemID == Zeuscraft.BowGolden.itemID)
        {
            ...
            if (k >= 18) return 27;//The return values are
            if (k >  13) return 28;//the icon indexes (in the /Tutorial/Items.png file)
            if (k >   0) return 29;
        }
        return 26;//getIconIndex(stack);
    }
